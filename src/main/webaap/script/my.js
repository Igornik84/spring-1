...
function edit_task(task_id){
    let indentifier_delete = "#delete_" + task_id;
    $(indentifier_delete).remove();

    let indentifier_edit = "#edit_" + task_id;
    let save_tag = "<button id = 'save_" + task_id + " '>Save</button>";
    $(indentifier_edit).html(save_tag);
    let property_save_tag = "update_task(" + task_id + ")";
    $(indentifier_edit).attr("onclick", property_save_tag);

    let current_tr_element = $(indentifier_edit).parent().parent();
...
function update_task(task_id){
    ...
    $.ajax({
        ...
        type: 'POST',
        dataType: 'json',
        containerType: 'application/json; charset = UTF-8',
        async: false,
